...
- **Mission Reconciliation & Edit**: The payout loop re-checks each due mission against Discord before paying. If the Discord scheduled event was deleted, set to `canceled`, or renamed so it no longer starts with `Actors Needed:`, the row is marked canceled (new `canceled` / `canceled_at` columns on `mission_event`) and no payout fires. If start/end time was edited, `mission_event.start_ts`/`end_ts`/`payout_ts` are synced and payout defers to the new midnight ET. The Missions panel has a new **Edit Mission** button (✏️ row 2): pick a recent mission → opens an edit panel with **Edit Date/Time** (modal, updates Discord event + recomputes payout_ts), **Edit Attendees** (UserSelect, replaces list), **Edit Payout** (modal, updates pay_per_player and rewrites event description), and **Cancel Mission** (deletes the Discord event + marks the row canceled). All edits go through `mission_event_update` / `mission_event_cancel`; reconciliation notices are posted back to the mission's origin channel.
