        ...
        fields = [
            ...
            (
                ...
                "\n".join([
                    ...
                    "**Edit Mission** (panel button) – the dropdown only lists **active** missions (not yet paid, not canceled). Pick one, then change date/time, attendees, or payout (Discord event + DB stay in sync), or cancel it entirely (deletes the Discord event and skips auto-payout). Edit Attendees has a `Credit Added` toggle (ON by default — newly-added attendees get a gig-log credit) and a `Reverse Removed` toggle (OFF by default — removed attendees lose one gig-log credit for the mission date). Cancel Mission has a `Reverse Gig-Log Credits` toggle (OFF by default — when ON, removes one gig-log credit per attendee). A 🔄 Refresh button re-reads the row and re-locks the panel if the mission was just paid or canceled by the background loop.",
