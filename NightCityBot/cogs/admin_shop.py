...
class AdminShopMenuView(SafeView):
    ...
    @discord.ui.button(label="Resync Gun Wholesaler", style=discord.ButtonStyle.primary, emoji="🏭", row=2, custom_id="admin_shop:resync_gun_wh")
    async def resync_gun_wh(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Regenerate gun wholesaler lots from the current Master Gun List sheet."""
        await interaction.response.defer(ephemeral=True)
        guns_cog = interaction.client.get_cog("GunsShopCog")
        if not guns_cog:
        ...
        try:
            guns = await guns_cog._load_master_guns()
            if not guns:
                await send_ephemeral(interaction, "❌ No guns found in the Master Gun List. Set the sheet URL first.")
                return
            async with guns_cog.lock:
                state = await guns_cog._load_state()
            ...
            if not save_ok:
                ...
                return
            total_qty = sum(int(l.get("qty_available", 0)) for l in new_lots)
            await send_ephemeral(interaction,
            ...
            logger.exception("resync_gun_wh failed")
            await send_ephemeral(interaction, f"❌ Resync failed: {e}")
            return
        cog = interaction.client.get_cog("AdminShop")
        if cog:
            log_ch = await cog._audit_channel()
            if log_ch:
                try:
                    await log_ch.send(
                        f"🏭 **Admin: Gun Wholesaler Resynced** — {interaction.user.display_name} ({interaction.user.id}) "
                        f"regenerated {len(new_lots)} lots from sheet."
                    )
                except Exception:
                    pass

    @discord.ui.button(label="Resync Cyberware Wholesaler", style=discord.ButtonStyle.primary, emoji="🏭", row=2, custom_id="admin_shop:resync_cw_wh")
    async def resync_cw_wh(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Refresh cyberware catalogue from sheet and rebuild wholesaler stock from it."""
        await interaction.response.defer(ephemeral=True)
        cw_cog = interaction.client.get_cog("CyberwareShop")
        if not cw_cog:
        ...
        try:
            ...
            sheet_url = str(cw_state.get("sheet_url", "")).strip()
            if not sheet_url:
                await send_ephemeral(interaction, "❌ No cyberware sheet URL configured. Use **Set Cyberware Sheet** first.")
                return
            await download_sheet(sheet_url, cw_cog.sheet_cache_path)
            items = parse_cyberware_sheet(cw_cog.sheet_cache_path)
            if not items:
                await send_ephemeral(interaction, "❌ No cyberware items parsed from sheet.")
                return
            await cw_catalog_upsert_many(items)
            await cw_cog._save_catalog(items)
            new_lots = []
            for item in items:
                new_lots.append({
                    "lot_id": f"cat-{item.get('name', '?')}",
                    "item_name": item.get("name", "?"),
                    "unit_cost": int(item.get("price", 0) or 0),
                    "cwp": item.get("cwp", ""),
                    "slot": item.get("slot", ""),
                    "qty_available": 99,
                })
        ...
        if cog:
            ...
            if log_ch:
                try:
                    await log_ch.send(
                        f"🏭 **Admin: Cyberware Wholesaler Resynced** — {interaction.user.display_name} ({interaction.user.id}) "
                        f"refreshed catalogue and rebuilt {len(new_lots)} wholesale lots from sheet."
                    )
                except Exception:
                    pass
